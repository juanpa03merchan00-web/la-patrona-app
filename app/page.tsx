...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-yellow-600/30">
      {/* CONTENEDOR PRINCIPAL: max-w-md limita el ancho en PC para que no se vea gigante */}
      <main className="max-w-md mx-auto border-x border-zinc-900 min-h-screen pb-80 shadow-2xl">
        
        {/* HEADER: Tamaño de logo controlado */}
          ...
          {licores.map((licor) => (
            <div key={licor.id} className="bg-zinc-900/40 rounded-2xl p-3 border border-zinc-800/50 flex flex-col items-center">
              <div className="h-32 w-full flex items-center justify-center mb-2">
                <img src={licor.img} alt={licor.nombre} className="max-h-full object-contain hover:scale-105 transition-transform" />
              </div>
        ...
        {/* FOOTER FIJO CON DATOS DE PAGO Y FORMULARIO */}
