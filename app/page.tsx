...
export default function Home() {
  ...
  return (
    // Degradado de fondo y contenedor centrado para que no se vea gigante en PC
    <div className="min-h-screen bg-gradient-to-b from-zinc-950 to-black overflow-x-hidden">
      <main className="max-w-md mx-auto border-x border-zinc-900 shadow-2xl pb-80 bg-black/50 min-h-screen">
        
        <header className="text-center py-8 flex flex-col items-center border-b border-yellow-600/10 mb-6 px-4">
          <div className="w-44 h-44 relative mb-4">
            <img 
              src="/logo-grande.jpg" 
              alt="Logo La Patrona" 
              className="rounded-full border-4 border-yellow-600 shadow-2xl object-cover w-full h-full"
            />
          </div>
          ...
          {licores.map((licor) => (
            <div key={licor.id} className="bg-zinc-900/40 rounded-2xl p-3 border border-zinc-800/50 flex flex-col items-center">
              ...
              <button 
                ...
                className="w-full bg-yellow-600 active:bg-yellow-700 text-black font-black py-2 rounded-xl text-[9px] uppercase transition-all"
              >
                + Agregar
              </button>
            </div>
        ...
        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-zinc-950/95 backdrop-blur-xl border-t border-yellow-600/30 p-5 z-50">
          <div className="space-y-4">
            <div className="bg-zinc-900/80 p-3 rounded-xl border border-yellow-600/10 text-center">
              ...
              <p className="text-xs text-zinc-100 font-mono italic">Ahorros: 0230142x00x</p>
            </div>
            ...
            <div className="flex justify-between items-center bg-black/40 p-1 rounded-full border border-zinc-800">
              ...
              <button onClick={enviarWhatsApp} className="bg-green-600 hover:bg-green-500 px-6 py-4 rounded-full font-black text-[10px] shadow-lg shadow-green-900/20 active:scale-95 transition-all">
                ENVIAR PEDIDO
              </button>
            </div>
